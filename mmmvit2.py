...
class MMVit2(nn.Module):
    def __init__(self, num_cls=1):
        super(MMVit2, self).__init__()
        ...
        self.qkv_RGB = nn.Conv3d(transformer_basic_dims, transformer_basic_dims * 3, kernel_size=1, stride=1, padding=0)
        self.qkv_NIR = nn.Conv3d(transformer_basic_dims, transformer_basic_dims * 3, kernel_size=1, stride=1, padding=0)
        self.qkv_SWIR = nn.Conv3d(transformer_basic_dims, transformer_basic_dims * 3, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        ########### InterFormer
        ########### Multimodal Correlation Modeling       
        temp_RGB = self.qkv_RGB(RGB_intra_x6)
        q_RGB, k_RGB, v_RGB = temp_RGB.chunk(3, dim=1)

        temp_NIR = self.qkv_NIR(NIR_intra_x6)
        q_NIR, k_NIR, v_NIR = temp_NIR.chunk(3, dim=1)
        
        temp_SWIR = self.qkv_SWIR(SWIR_intra_x6)
        q_SWIR, k_SWIR, v_SWIR = temp_SWIR.chunk(3, dim=1)
        
        para_RGB = torch.cat([(q_RGB*k_RGB).view(1, -1), (q_RGB*k_NIR).view(1, -1), (q_RGB*k_SWIR).view(1, -1)], dim=0)
        #print("para_RGB shape", para_RGB.shape)
        a1 = self.softmax_RGB(para_RGB/np.sqrt(3)).view(RGB_intra_x6.size(0), RGB_intra_x6.size(1)*3, RGB_intra_x6.size(2), RGB_intra_x6.size(3), RGB_intra_x6.size(4))
        x6_RGB_ = a1[:, 0:RGB_intra_x6.size(1), :, :, :] * v_RGB + a1[:, RGB_intra_x6.size(1):RGB_intra_x6.size(1)*2, :, :, :] * v_NIR + a1[:, RGB_intra_x6.size(1)*2:RGB_intra_x6.size(1)*3, :, :, :] * v_SWIR 
        #print("x6_RGB_ shape", x6_RGB_.shape)

        para_NIR = torch.cat([(q_NIR*k_RGB).view(1, -1), (q_NIR*k_NIR).view(1, -1), (q_NIR*k_SWIR).view(1, -1)], dim=0)
        a2 = self.softmax_NIR(para_NIR/np.sqrt(3)).view(NIR_intra_x6.size(0), NIR_intra_x6.size(1)*3, NIR_intra_x6.size(2), NIR_intra_x6.size(3), NIR_intra_x6.size(4))
        x6_NIR_ = a2[:, 0:NIR_intra_x6.size(1), :, :, :] * v_RGB + a2[:, NIR_intra_x6.size(1):NIR_intra_x6.size(1)*2, :, :, :] * v_NIR + a2[:, NIR_intra_x6.size(1)*2:NIR_intra_x6.size(1)*3, :, :, :] * v_SWIR 
        
        para_SWIR = torch.cat([(q_SWIR * k_RGB).view(1, -1), (q_SWIR * k_NIR).view(1, -1), (q_SWIR * k_SWIR).view(1, -1)], dim=0)
        a3 = self.softmax_SWIR(para_SWIR/np.sqrt(3)).view(SWIR_intra_x6.size(0), SWIR_intra_x6.size(1) * 3, SWIR_intra_x6.size(2), SWIR_intra_x6.size(3), SWIR_intra_x6.size(4))
        x6_SWIR_ = a3[:, 0:SWIR_intra_x6.size(1), :, :, :] * v_RGB + a3[:, SWIR_intra_x6.size(1):SWIR_intra_x6.size(1)*2, :, :, :] * v_NIR + a3[:, SWIR_intra_x6.size(1)*2:SWIR_intra_x6.size(1)*3, :, :, :] * v_SWIR         
        
        x6_intra_ = torch.stack((x6_RGB_, x6_NIR_, x6_SWIR_), dim=1).view(x.size(0),-1, patch_size, patch_size, patch_size)
        ########### Multimodal Correlation Modeling

        ########### Multimodal Representation Fusion
        RGB_intra_x6, NIR_intra_x6, SWIR_intra_x6 = torch.chunk(x6_intra_, num_modals, dim=1)
